...
void TankerkoenigModule::resetPaging() {
...
unsigned long TankerkoenigModule::getDisplayDuration() {
    // Berechne Gesamtdauer basierend auf Anzahl Stationen * Dauer pro Station
...
void TankerkoenigModule::tick() {
    // Fordert Neuzeichnung an für flüssige Animationen
...
void TankerkoenigModule::setConfig(const PsramString& apiKey, const PsramString& stationIds, int fetchIntervalMinutes, unsigned long pageDisplaySec) {
    ...
    _currentTicksPerPage = _pageDisplayDuration / 100; // z.B. 15000ms / 100ms = 150 Ticks
    if (_currentTicksPerPage == 0) _currentTicksPerPage = 1; // Mindestens 1 Tick
    ...
    if (_isEnabled) {
        ...
        webClient->registerResource(String(this->resource_url.c_str()), fetchIntervalMinutes, nullptr);
    } else {
...
void TankerkoenigModule::begin() {
    ...
    // IMMER laden (unabhängig vom SAVE_HISTORY Flag)
    loadPriceStatistics();
        ...
        for (const auto& id : id_list) {
            if (prices[id.c_str()].is<JsonObject>()) {
                ...
                if (is_open) {
                    ...
                    // IMMER Statistiken aktualisieren (Speichern wird intern geprüft)
                ...
                // IMMER Trends berechnen (unabhängig vom Speichern)
...
void TankerkoenigModule::draw() {
    ...
    if (data.lastPriceChange > 0) {
        ...
        struct tm timeinfo; 
        gmtime_r(&local_change_time, &timeinfo);
...
void TankerkoenigModule::drawTrendArrow(int x, int y, PriceTrend trend) {
    const int anim_cycle = 1200;
    const int pattern_height = 4;
    const int visible_height = 8;
