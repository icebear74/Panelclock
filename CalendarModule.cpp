...
void CalendarModule::setICSUrl(const PsramString &url) {
    this->icsUrl = url;
    if (!icsUrl.empty()) webClient->registerResource(String(icsUrl.c_str()), fetchIntervalMinutes, root_ca_pem);
}

void CalendarModule::setFetchIntervalMinutes(uint32_t minutes) { 
    fetchIntervalMinutes = minutes > 0 ? minutes : 60; 
}
...
void CalendarModule::tickScroll() {
...
void CalendarModule::setScrollStepInterval(uint32_t ms) { 
...
void CalendarModule::setColors(const PsramString& dateColorHex, const PsramString& textColorHex) {
